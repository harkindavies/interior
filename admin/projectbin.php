...
$deleteproject = "";
  if (isset($_SESSION['deleteproject'])) {
    ...
    elseif($deleteproject == 2){
     ...
     elseif($deleteproject == 3){
      ?>
      	<div class="container-fluid">
			<div class="row">
				<div class="col-lg-4 col-md-4 col-sm-8 col-xs-12">
					<div class="ordermsg" id="ordermsg">
						<span>Project successfully retrieved.</span>
						<div id="myProgress" class="myProgress" >
								<div id="myBar"></div>
						</div>
					</div>
     ...
     elseif($deleteproject == 4){
      ?>
      	<div class="container-fluid">
			<div class="row">
				<div class="col-lg-4 col-md-4 col-sm-8 col-xs-12">
					<div class="ordermsg ordermsg2" id="ordermsg">
						<span> Error occur while retrieving the project.</span>
						<div id="myProgress" class="myProgress2" >
								<div id="myBar"></div>
						</div>
					</div>
...
//unset($_SESSION['deleteproject']);
 ...
 $rslt = mysqli_fetch_assoc($selected); 
  ...
  if($position == 'CEO & Founder'){
   ...
   $total_pages = $conn->query("SELECT * FROM deletedproject ")->num_rows;
...
// Number of results to show on each page.
 $num_results_on_page = 4;
  if ($stmt = $conn->prepare("SELECT * FROM deletedproject ORDER BY sn desc LIMIT ?,?")) { 
		...
		<div class="container-fluid">
			<div class="row" id="addpromove">
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					<a href="viewproject.php"><button class="choosepro " id="addpicture">View Project <span class="fa fa-photo" style="margin-left: 10px;"></span></button></a>
					<?php if ($remove == 1){ ?>
					<a href="projectbin.php"><button class="choosepro chooseactive" id="viewbin">Project Bin<span class="fa fa-trash-o" style="margin-left: 10px;"></span></button></a>
					<?php } ?>
	           ...
	           <tr>
		          <th>S/N</th>
        			...
        			<?php while ($row = $result->fetch_assoc()): ?> 
                      ...
                      <a title="click to delete" href="retrieveproject.php?sn=<?php echo $row ['sn'];?>" onclick="return confirm ('Are you sure you want to retrieve <?php echo $row ['projectname']; ?> from deleted projects ?')" ><button id="retrieve" class="btn btn-success fa fa-download"> Restore Project </button></a>
                    </td>
			            <td style="text-align: center;">
                      <a title="click to delete" href="pdeleteproject.php?sn=<?php echo $row ['sn'];?>" onclick="return confirm ('Are you sure you want to delete <?php echo $row ['projectname']; ?> from project ?')" ><button id="deletebtn" class="btn btn-danger fa fa-trash"> Delete Project </button></a>
                    </td>
		       	  	</tr>
         			<?php endwhile; ?>
				</table>
